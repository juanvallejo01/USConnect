...
export function UsersTab() {
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      ...
      {userList.map((user) => (
        <div key={user.id} className="rounded-xl bg-card border border-border shadow-sm p-4">
          <div className="flex items-start justify-between mb-2">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h4 className="text-sm font-semibold text-foreground">{user.name}</h4>
                {user.verified && <ShieldCheck size={14} className="text-[#1565C0] shrink-0" />}
              </div>
          ...
          <div className="flex items-center gap-2 mt-3">
            {!user.verified && (
              <button
                ...
                className="flex items-center gap-1.5 rounded-lg bg-[#1565C0]/10 px-3 py-1.5 text-xs font-medium text-[#1565C0] transition-colors hover:bg-[#1565C0]/20 active:scale-95"
              >
